...
- **Large `k` (editing count).** Edit more trajectories. `k` is a selection-count knob, not an algorithm change — no sign-off needed. **Empirically (2026-05-28), under `--max-per-unit 1` this lever saturates at ~1,186 edits: `k=10000` still selected only 1,186** — the count of distinct negative-attribution units that contain a relocatable pickup (2,829 units are negative, but ~1,643 have no pickup to move). So `k` above ~1,186 is inert here; the diversity budget binds, not `k`. ΔF_causal at the cap = +0.0093 (vs +0.0087 at k=1000). To push past the cap, use the gated coordinate-descent rounds (below) or relax `--max-per-unit` (pile-up risk per §8.1/§8.3). *(Convention: lowercase `k` = editing selection count; uppercase `K` = filtering removal count in B2 — kept distinct throughout this spec.)*
