...
| `shz_a10_wbc_n12_pooled_stats.json` | pooled: `weighted_bc_sweep/alpha_sweep_s10_c80_f10_filtered_6seed/` + `alpha_sweep_s10_seeds6to11/` | §4.4 FLAGSHIP n=12: edited_w30 **+0.0297±0.0029, 12/12, exact Wilcoxon p=.00049** (floor; t_p 1e-12; CI [+0.0278,+0.0315]) — survives Holm; controls null at n=12 (random −0.0017 4/12, most-fair +0.0012 7/12); vanilla +0.0016 7/12 n.s. → §4.1 protocol note + §5 concession carve-out (WBC-N12, 2026-07-18) |
...
| `sf12_a10_wbc_dose_ext_paired_stats.json` | `weighted_bc_sweep/supply_lift_a10_sf12_dose_ext_6seed/` | §4.7 SF saturation: w40 +0.0370 / w50 +0.0371 (6/6, flat past w40); random n.s. negative; **⚠️ most-fair selection turns SIG-POSITIVE at extended doses (+0.0063/+0.0066, ~⅙ of edited)** — dominated-control pattern, surfaced (SF-WBC-DOSEEXT, 2026-07-17) |
| `sf12_a10_wbc_n12_pooled_stats.json` | pooled: `weighted_bc_sweep/supply_lift_a10_sf12_filtered_6seed/` + `supply_lift_a10_sf12_seeds6to11/` | §4.7 SF FLAGSHIP n=12: edited_w30 **+0.0333±0.0050, 12/12, exact Wilcoxon p=.00049** (floor, both cities now); ⚠️ n=12 controls BOTH significant: random −0.0077 (2/12, p=.0024) NEGATIVE; most-fair +0.0059 (10/12, p=.0020) positive-but-dominated (~⅙) — surfaced in §4.7; vanilla +0.0017 n.s. (SF-WBC-N12, 2026-07-18) |
| `sf12_a10_rollout_summary.json` | `external_fairness/results/option_a_rollout_sf12_tl/` | §4.7 SF allocation boundary: trim+lift −0.0335 @ w30 (0/6, p=.031) — ~10× the SZ magnitude (C3-SF-ROLLOUT-TL, 2026-07-17) |
| `sf12_trimonly_a10_rollout_summary.json` | `external_fairness/results/option_a_rollout_sf12_trimonly/` | §4.7 SF demand-only rollout comparator: −0.0432 @ w30 (0/6, p=.031) → **~22% attenuation like-for-like, not a reversal** — boundary replicates cross-city (C3-SF-ROLLOUT-TO, 2026-07-17) |
...
| `PAPER/baselines/fairness-intervention/fb_reweigh_paired_stats.json` | `weighted_bc_sweep/fairness_baseline_6seed/` | §4.5 training-side baseline: Kamiran-Calders-style reweigh on raw **−0.0227 F_causal, 6/6 negative, p=.031**; Fid-B +0.0144 ≈ w30's cost; edited_w30 +0.0302 replicated internally (FB-REWEIGH, 2026-07-17) |
| `PAPER/baselines/fairness-intervention/fb_penalty_paired_stats.json` + `_probe_sweep.json` | `weighted_bc_sweep/fairness_penalty_{6seed,probe}/` | §4.5 in-processing penalty: inert at λ∈{1,3.16,10} n=6 (\|Δ\|≤0.0009 n.s.) + λ=100 probe; catastrophic at λ=1000 (F_causal −0.2053, Fid-B 0.514) — no constructive operating range (FB-PENALTY + FB-PENALTY-PROBE, 2026-07-17) |
| `PAPER/baselines/fairness-intervention/fb_penalty_abs_{paired_stats,pilot_sweep}.json` | `weighted_bc_sweep/fairness_penalty_abs_{6seed,pilot}/` | §4.5 formulation closure: ABSOLUTE penalty tracks signed at every dose — pilot point-by-point on the signed grid; n=6 l10 +0.0008 n.s. (inert), l1000 **−0.1293 6/6, Fid-B 0.363** (destructive; shallower than signed's −0.2053/0.514 = post-zero push-back) → "in this formulation" scoping DROPPED, no constructive range under either formulation (FB-PENALTY-ABS, 2026-07-18) |
| `PAPER/baselines/fairness-intervention/fb_rollout_summary.json` | `external_fairness/results/option_a_rollout_fb/` | §4.5 rollout axis: fair_reweigh allocation drain −0.0010 (0/6, p=.031), penalty n.s.; **R5 arm-prefix regression gate PASSED** (edited_w30 −0.00329 ≈ committed −0.0033) (FB-ROLLOUT, 2026-07-17; first launch ABORTED era-wrong — see ledger) |
...
| `shz_hgc_a10_channel_decomposition.json` + `_supply_recount.json` | `2026-07-13T04-41-12_..._hgc_filtered/` | §4.6 lift-up CI: total +0.0594 [+0.0181,+0.1013]; supply t1 +0.0054 sig, **TIER-2 +0.0211 [+0.0181,+0.0247] sig** (A1' 2026-07-17); demand +0.0539 sig (unlike PRIMARY) |
...
| `shz_4feat_a10_channel_decomposition.json` + `_supply_recount.json` | `2026-07-13T17-04-22_..._4feat_filtered/` | §4.6 lift-up CI: total +0.1461 [+0.0900,+0.2039]; supply t1 +0.0608 sig, **TIER-2 +0.0771 [+0.0706,+0.0836] sig** (A1' 2026-07-17); demand +0.0853 sig — explains the 2.8× PRIMARY total |
