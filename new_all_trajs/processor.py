...
def compute_global_bounds(all_data: Dict[str, Dict[str, List]]) -> GlobalBounds:
    """
    Compute global GPS bounds from all raw data.
...
def gps_to_grid(
    ...
    Uses the same quantization logic as other datasets in the project.
    ...
    """
    lat_bins = np.arange(bounds.lat_min, bounds.lat_max + config.grid_size, config.grid_size)
    lon_bins = np.arange(bounds.lon_min, bounds.lon_max + config.grid_size, config.grid_size)
    
    x_grid = np.digitize(lat, lat_bins) - 1 + config.x_grid_offset
    y_grid = np.digitize(lon, lon_bins) - 1 + config.y_grid_offset
...
def process_data(
    ...
    for i, filename in enumerate(config.input_files):
        ...
        if filepath.exists():
            try:
                all_raw_data[filename] = load_raw_data(filepath)
                update_progress(f"Loaded {filename}", (i + 1) / len(config.input_files) * 0.1)
    ...
    update_progress("Computing global bounds...", 0.15)
    bounds = compute_global_bounds(all_raw_data)
    stats.global_bounds = bounds
    
    # Stage 4: Build driver index and collect all plates
    update_progress("Building driver index...", 0.2)
    all_plates = set()
    for month_data in all_raw_data.values():
        all_plates.update(month_data.keys())
    ...
    # Stage 5: Process each driver's data
    update_progress("Processing trajectories...", 0.25)
    all_trajs = {idx: [] for idx in range(len(sorted_plates))}
    total_records = 0
    total_states = 0
    ...
    for plate_idx, plate in enumerate(sorted_plates):
        ...
        # Combine records from all months for this driver
        # Data structure: month_data[plate] = [[records for day1], [records for day2], ...]
        driver_records = []
        for month_data in all_raw_data.values():
        ...
        # Update progress
        progress = 0.25 + (plate_idx / len(sorted_plates)) * 0.7
