...
share. These covariates resolve at coarse administrative units (districts or
census tracts), which bounds the
resolution of any demographic analysis built on them (\S\ref{sec:objective}).
% 2026-07-25 (review finding #15): "at district granularity" was Shenzhen's fact
% stated globally — \S\ref{sec:exp-setup} fills San Francisco's features from ACS
% TRACT values (PAPER/argument/02_datasets.md: SZ = 10 district profiles; SF = ACS
% 2006-2010 tracts -> cells). The replacement is true of both cities and keeps the
% resolution caveat; the per-city detail stays in \S\ref{sec:exp-setup}.
...
only a small share of the corpus may change. The difficulty lies in choosing where to spend
...
\textbf{C3 (de\-mand-ad\-just\-ed fairness).}
% 2026-07-25: discretionary hyphens added to the bold compound. Without them
% "demand-adjusted" is unbreakable and the challenge block overran the column by
% 6.57pt (lint gate is >5pt). Same house pattern as "demo\-graph\-ic-parity" and
% "dis\-pa\-rate-impact" elsewhere; the rendered word is unchanged.
% 2026-07-25 (later, found during the fig-2 fork-removal pass): the comment above
% had swallowed the sentence opening "Equal service is not the target: a" (same
% defect class as commit 6d03679), so C3 rendered as "fairness).business district
% should...". Restored below; the % after \textbf{} is dropped so the line break
% supplies the space the swallowed text lost.
Equal service is not the target: a
business district should receive more service than a quiet residential block.
The objective must score only the service variation demand leaves
unexplained (\S\ref{sec:objective}).
