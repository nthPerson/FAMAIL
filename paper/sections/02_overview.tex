...
data (HSTD): here, a \textbf{corpus} $\mathcal{T} = \{\tau\}$ of real
...
The imitation we demonstrate on this corpus (\S\ref{sec:downstream}) is
supervised: there is no reward function, and the recorded state transitions
are the only training signal. A policy $\pi$ is trained by behavior cloning to
...
($N \approx 34{,}500$ on the primary city). Each active unit carries a
...
level of individual trajectories: an edited trajectory must remain
recognizable to a frozen driver-identity discriminator as its original
driver; and (iii) transfers its fairness into a policy trained on it by
...
perturbed within an $L_\infty$ ball of radius $\varepsilon = 2$ grid cells.
...
discarding the biased slice or regenerating the corpus discards the asset: the
repair must edit the data we already have, and only a small share of it. The
difficulty is then where to spend a budget of $k \ll |\mathcal{T}|$ edits so
that a small portion of the trajectories moves the fairness of the whole
corpus (\S\ref{sec:editor}).
\textbf{C2 (fidelity).} A fairness edit is worthless if the result no longer
resembles real driving: an edited trajectory must still read as its original
driver. The budget is therefore spent under constraints rather than freely,
with every edit bounded to a small neighborhood of the recorded trajectory
and scored by a frozen driver-identity discriminator inside the editing
objective (\S\ref{sec:objective}).
...
objective must therefore score only the service variation that demand does
not explain (\S\ref{sec:objective}).
\textbf{C4 (level up, not down).} Measured fairness can improve while the
under-served gain nothing, and a demand-only editor does exactly this on our
data, redistributing service among better-served areas
(\S\ref{sec:leveling}). Leveling up instead needs a second channel that adds
real taxi presence where it is missing.
\textbf{C5 (surviving training).} Sparse edits are averaged away by training
that weights every demonstration equally, and the recovered fairness never
reaches the policy. The influence of the edited demonstrations must be
preserved during training, and the recovered fairness traced to the edits
rather than to the reweighting itself (\S\ref{sec:downstream}).
