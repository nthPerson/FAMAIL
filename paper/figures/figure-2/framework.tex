...
%   canvas 8.40 x 10.16 cm; MEASURED box (framework-test.tex prints it):
%   239.80pt wide x 289.88pt tall (re-measured 2026-07-25 after the Stage-2
%   fork removal; the "8.40 x 10.54 / 300.69pt" previously recorded here was
%   stale from before an earlier canvas shrink). Gates: \columnwidth (acmart
%   sigconf) = 241.15pt -> 1.3pt to spare; 0.5\textheight = 313.0pt ->
%   23.1pt to spare.
...
%   BAND LABELS (2026-07-25, Robert): the three drawn bands are labelled
%   "Stage 1a: Attribute", "Stage 1b: Trim + Lift", "Stage 2: Upweight" —
%   NOT 1/2/3. FATE has TWO stages (\S3's opening sentence, \S6, and the
%   two-stage claim \S4 tests): a budgeted constrained EDIT, then edit-aware
%   WEIGHTING. Attribution is not a third stage; \S3.2 calls it "the mechanism
%   that decides which trajectories receive the budget", i.e. part of the edit.
%   The a/b split keeps attribute and edit visually distinct (they answer
%   different challenges, C1 and C2/C4) while showing the nesting. The geometry
%   comments below still say "stage-1/2/3 band" for the three BANDS; read those
%   as band indices, not stage names.
...
%   fwpos is used ONLY in the terminal outcome box (the exemplar's green
%   "Correct Label!" analogue); the check mark carries the verdict without
%   color. (fwneg + the xmark pic left with the uniform-weights fork,
%   removed 2026-07-25 -- see the Stage-2 comment block.)
%
...
\begin{tikzpicture}[x=1cm, y=1cm, line cap=round, line join=round,
  ...
  trajc/.pic={\draw (0,0.14) -- (0.14,0.03) -- (0.30,0.12) -- (0.44,0.04)
                    -- (0.60,0.15);},
  % ---- terminal verdict mark (drawn, so no extra package is needed) ----
  ...
  \def\bL{2.20}          % left  branch axis (trim panel, Stage 1b)
  \def\bR{6.20}          % right branch axis (lift panel, Stage 1b)
  ...
  % =====================================================================
  % STAGE 1a -- Attribute: score every trajectory, select the budgeted few
  % =====================================================================
  \node[ttl] at (0.20,-1.59)
    {\textbf{Stage 1a: Attribute}\\\textcolor{fwgray}{select what to edit}};
  \draw[flow] (\spine,-1.47) -- (\spine,-1.63);
  % 2026-07-25: box widened to the right (5.88 -> 6.45) and its text re-centred on
  % the box (4.49) instead of the spine. The two-line label is ~3.67cm wide but the
  % box was 3.35cm, so the left border struck through the "c" of "score" and the
  % text ran back toward the band title. Height is unchanged, so the figure's
  % vertical rhythm and both size gates are untouched; the flow arrows still enter
  % and leave on the spine, which lies inside the wider box. An off-spine op-box is
  % already this figure's convention (Stage 1b's sits at 2.88-7.83, centred 5.355).
  \draw[opbox] (2.53,-1.63) rectangle (6.45,-2.18);
  \node[align=center] at (4.49,-1.905)
  ...
  % =====================================================================
  % STAGE 1b -- Trim + Lift: bounded edits to the selected trajectories
  % =====================================================================
  \node[ttl] at (0.20,-3.63)
    {\textbf{Stage 1b: Trim + Lift}\\\textcolor{fwgray}{bounded fairness edits}};
  ...
  % --- output of Stage 1b: the edited corpus ----------------------------
  ...
  % =====================================================================
  % STAGE 2 -- Upweight: the sparse edits must survive training
  % =====================================================================
  % 2026-07-25 (Robert): the uniform-weights fork (uniform weights ->
  % trained policy -> bias reproduced) is REMOVED -- it was the experimental
  % control, not part of the proposed method, so the band now shows only
  % FATE's own path, centred on the spine. The averaged-away null the red
  % terminal carried survives as the weight chip's second line, which keeps
  % the 2026-07-24 caption decision (that clause was dropped from the caption
  % BECAUSE the figure states it) sound. The freed width pays for the weight
  % chip: a trim/lift-style visual in which each demonstration stands on a
  % bar whose height is its loss weight -- edited (dashed amber) demos ride
  % tall amber bars, unchanged (gray) demos sit on short gray ones. Bar
  % HEIGHT carries the distinction, so the chip is grayscale-safe, and no
  % weight symbol is introduced (\S3.4 names none; symbol guard holds).
  \node[ttl] at (0.20,-7.45)
    {\textbf{Stage 2: Upweight}\\
     \textcolor{fwgray}{edit-aware imitation training}};
  \draw[flow] (\spine,-7.33) -- (\spine,-7.51);
  % op-box sits right of the title block (x > 2.9 rule), like Stage 1a's
  \draw[opbox, draw=fwsel, line width=0.7pt] (2.95,-7.51) rectangle (6.85,-8.06);
  \node[align=center] at (4.90,-7.785)
    {imitation training with the $k$ edited\\demonstrations upweighted in the loss};
  \draw[flow] (\spine,-8.06) -- (\spine,-8.20);
  % --- weight chip: per-demonstration loss weight drawn as support bars ---
  \draw[panel] (\xL,-8.20) rectangle (\xR,-8.82);
  \draw[fwrule, line width=0.5pt] (0.62,-8.74) -- (4.06,-8.74);
  \fill[fwsel] (0.74,-8.74) rectangle (1.30,-8.44);
  \pic[tedit] at (0.72,-8.42) {traja};
  \fill[fwsel] (1.40,-8.74) rectangle (1.96,-8.44);
  \pic[tedit] at (1.38,-8.42) {trajc};
  \fill[fwgray!55] (2.06,-8.74) rectangle (2.62,-8.62);
  \pic[tnone] at (2.04,-8.60) {trajb};
  \fill[fwgray!55] (2.72,-8.74) rectangle (3.28,-8.62);
  \pic[tnone] at (2.70,-8.60) {trajc};
  \fill[fwgray!55] (3.38,-8.74) rectangle (3.94,-8.62);
  \pic[tnone] at (3.36,-8.60) {traja};
  \node[anchor=west, align=left] at (4.22,-8.51)
    {\textbf{loss weight:} the $k$ edited count more,\\
     so the sparse edit is not averaged away};
  \draw[flow] (\spine,-8.82) -- (\spine,-8.96);
  \draw[data] (2.55,-8.96) rectangle (5.85,-9.32);
  \node at (\spine,-9.14) {\textbf{imitation learning model}};
  \draw[flow] (\spine,-9.32) -- (\spine,-9.46);
  % terminal outcome box (the exemplar's green "Correct Label!" analogue)
  \draw[fill=white, draw=fwpos, line width=0.8pt]
    (2.55,-9.46) rectangle (5.85,-10.08);
  \pic[fwpos] at (2.80,-9.77) {ckmark};
  \node[anchor=west, align=left, text=fwpos] at (2.98,-9.77)
